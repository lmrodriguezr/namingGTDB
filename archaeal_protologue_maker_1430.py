...
"""
archaeal_protologue_maker creates protologues from renamed and original GTDB metadata files
...
with open("ar53_metadata_r207_renamed.tsv",encoding='latin-1') as renamed_metadata:
...
with open("./input_files/ar53_metadata_r207.tsv",encoding='latin-1') as original_metadata:
  ...
  if "!" in current_line[1]: 
      if current_line[1].replace("p__", "").replace("ota!", "a") in current_line[5].replace("g__", ""): 
       if current_line[1] not in previous_line[1]:
          ...
          current_line[5].replace("g__", "").replace("!", ""),".\\par"\
          "\\pard \\sa480 According to GTDB R06-RS202, this phylum belongs to the domain \i Archaea \i0" \
          ". \\par",\
          file=protologues)
