...
A program to create protologues for bacterial taxa using information from renamed and original GTDB metadata files
Outputs a file of traditional protologues in Rich Text Format.
USAGE:
    python3 bacterial_protologue_maker.py 
...
with open("bac120_metadata_renamed.tsv",encoding='latin-1') as renamed_metadata:
...
with open("bac120_metadata_r202.tsv",encoding='latin-1') as original_metadata:
...
protologues = open("bacterial_protologues.rtf", "w")
...
## outputs a protologue for a newly named species
  ...
  if "!" in current_line[7]: 
      if current_line[7] not in previous_line[7]: ##ensures only first occurence of name acted on
        if "!" not in current_line[5]: ## selects newly named species in already named genus
          ...
          if "_" in current_line[5].replace("__", ""): 
            ...
            "(NL. fem. n. \i", current_line[7].replace("!", ""), "\i0 a species epithet created arbitrarily, while preserving the phonotactics and morphology of Latin; used as a noun in apposition). \\par", \
            "\\pard \\sa240 A species identified and delineated according to the algorithms of the Genome Taxonomy Database (GTDB) release R06-RS202."\
            "The GTDB R06-RS202 placeholder designation for this species is \i{\doccomm oldname_6}", \
            ...
            ## applies to species with well-formed genus names
            ## outputs relevant protologue
            print \
            ("\\pard \\sa120 \\b Description of \i {\doccomm newname_6}Candidatus \i0" + current_line[5].replace("!", "").replace("g__", ""), current_line[7].replace("!", ""), "sp. nov.{\doccomm 6_newname} \\b0 \\par", \
            "\\pard \\sa120 \i Candidatus \i0", current_line[5].replace("!", "").replace("g__", ""), current_line[7].replace("!", ""), \
            "(NL. fem. n. \i", current_line[7].replace("!", ""), "\i0 a species epithet created arbitrarily, while preserving the phonotactics and morphology of Latin; used as a noun in apposition). \\par", \
            "\\pard \\sa240 A species identified and delineated according to the algorithms of the Genome Taxonomy Database (GTDB) release R06-RS202."\
            "The GTDB R06-RS202 placeholder designation for this species is \i{\doccomm oldname_6}", \
            current_line[13].replace("g__", ""), current_line[15].replace("!", "") + \
          ...
          ## applies to newly named species in a newly named genus
          ## outputs relevant protologue
          print\
          ...
          "(NL. fem. n. \i", current_line[7].replace("!", ""), "\i0 a species epithet created arbitrarily, while preserving the phonotactics and morphology of Latin; used as a noun in apposition). \\par", \
          "\\pard \\sa240 A species identified and delineated according to the algorithms of the Genome Taxonomy Database (GTDB) release R06-RS202. The GTDB R06-RS202 placeholder designation for this species is \i{\doccomm oldname_6}", \
          current_line[13].replace("g__", ""), current_line[15].replace("!", "") + \
          "{\doccomm 6_oldname}\i0. The type genome for the species is the GenBank assembly", current_line[16].replace("GB_","") + \
  ...
  if "!" in current_line[5]: 
      if current_line[5] not in previous_line[5]:
        if "!" in current_line[4]: ## belongs to a newly named taxon, so old and new names of the higher taxon are included in protologue
          print \
          ...
          "\\pard \\sa120 \i Candidatus \i0", current_line[5].replace("!", "").replace("g__", ""), "(NL. fem. n. \i", current_line[5].replace("!", "").replace("g__", ""),\
          "\i0 a name created arbitrarily, while preserving the phonotactics and morphology of Latin). \\par", \
          "\\pard \\sa120 A taxon identified and delineated according to the algorithms of the Genome Taxonomy Database (GTDB) release R06-RS202. The GTDB R06-RS202 placeholder designation for this genus is \i{\doccomm oldname_5}", \
          ...
          "\\pard \\sa480 According to the original version of GTDB R06-RS202, this genus belongs to the family \i " + current_line[12].replace("f__", "").replace("!","") + \
          "\i0. According to the renamed version of GTDB R06-RS202, this taxon belongs to the family \i Candidatus \i0" + current_line[4].replace("f__", "").replace("!","") + \
          ". \\par",\
          file=protologues)
        else:          ## belongs to an already named taxon, so no need to include names of the higher taxon from the original and renamed taxonomies in the protologue
          print \
          ("\\pard \\sa120 \\b Description of \i {\doccomm newname_5}Candidatus \i0" + current_line[5].replace("g__", "").replace("!", ""), "gen. nov.{\doccomm 5_newname} \\b0 \\par", \
          "\\pard \\sa120 \i Candidatus \i0", current_line[5].replace("!", "").replace("g__", ""), "(NL. fem. n. \i", current_line[5].replace("!", "").replace("g__", ""),\
          "\i0 a name created arbitrarily, while preserving the phonotactics and morphology of Latin). \\par", \
          ...
          current_line[13].replace("g__", "") + \
           "{\doccomm 5_oldname}\i0. The type species for the genus is \i Candidatus \i0",\
          current_line[5].replace("g__", "").replace("!", ""), current_line[7].replace("!", ""),".\\par" \
          "\\pard \\sa480 According to GTDB R06-RS202, this genus belongs to the family \i" + current_line[12].replace("f__", "").replace("!","") + \
          "\i0. \\par",\
          file=protologues)
  ...
  if "!" in current_line[4]: 
    if current_line[4].replace("f__", "").replace("aceae!", "a") in current_line[5].replace("g__", ""):
      if current_line[4] not in previous_line[4]:
        if "!" in current_line[3]: ## belongs to a newly named taxon, so old and new names of the higher taxon are included in protologue
          print \
          ...
          "\\pard \\sa120 \i Candidatus \i0", current_line[4].replace("!", "").replace("f__", ""), "(NL. fem. n. \i", current_line[4].replace("!", "").replace("f__", ""),\
          "\i0 a name created from the name of the type genus by addition of an appropriate suffix). \\par", \
          "\\pard \\sa120 A taxon identified and delineated according to the algorithms of the Genome Taxonomy Database (GTDB) release R06-RS202. The GTDB R06-RS202 placeholder designation for this taxon is \i{\doccomm oldname_4}", \
          current_line[12].replace("f__", "") + \
          "{\doccomm 4_oldname}. \i0 The type genus for the taxon is \i Candidatus \i0",\
          current_line[5].replace("g__", "").replace("!", ""),".\\par"\
          "\\pard \\sa480 According to the original version of GTDB R06-RS202, this family belongs to the order \i " + current_line[11].replace("o__", "").replace("!","") + \
          "\i0. According to the renamed version of GTDB R06-RS202, this taxon belongs to the order \i Candidatus \i0" + current_line[3].replace("o__", "").replace("!","") + \
          ". \\par",\
          file=protologues)
        else:          ## belongs to an already named taxon, so no need to include names of the higher taxon from the original and renamed taxonomies in the protologue
          print \
          ...
          "{\doccomm 4_oldname}. \i0 The type genus for the taxon is \i Candidatus \i0",\
          current_line[5].replace("g__", "").replace("!", ""),".\\par"\
          "\\pard \\sa480 According to GTDB R06-RS202, this family belongs to the order \i " + current_line[3].replace("o__", "").replace("!","") + \
          "\i0. \\par",\
          file=protologues)
  ...
  if "!" in current_line[3]: 
      if current_line[3].replace("o__", "").replace("ales!", "a") in current_line[5].replace("g__", ""): 
         if current_line[3] not in previous_line[3]:
          if "!" in current_line[2]: ## belongs to a newly named taxon, so old and new names of the higher taxon are included in protologue
            print \
            ...
            "\\pard \\sa120 \i Candidatus \i0", current_line[3].replace("!", "").replace("o__", ""), "(NL. fem. n. \i", current_line[3].replace("!", "").replace("o__", ""),\
            "\i0 a name created from the name of the type genus by addition of the appropriate suffix). \\par", \
            "\\pard \\sa120 A taxon identified and delineated according to the algorithms of the Genome Taxonomy Database (GTDB) release R06-RS202. The GTDB R06-RS202 placeholder designation for this taxon is \i{\doccomm oldname_3}", \
            current_line[11].replace("o__", "") + \
            "{\doccomm 3_oldname}. \i0 The type genus for the taxon is \i Candidatus \i0",\
            current_line[5].replace("g__", "").replace("!", ""),".\\par"\
            "\\pard \\sa480 According to the original version of GTDB R06-RS202, this family belongs to the class \i " + current_line[10].replace("c__", "").replace("!","") + \
            "\i0. According to the renamed version of GTDB R06-RS202, this taxon belongs to the class \i Candidatus \i0" + current_line[2].replace("c__", "").replace("!","") + \
            ". \\par",\
            file=protologues)
          else:  ## belongs to an already named taxon, so no need to include names of the higher taxon from the original and renamed taxonomies in the protologue
            print \
            ...
            "{\doccomm 3_oldname}. \i0 The type genus for the taxon is \i Candidatus \i0",\
            current_line[5].replace("g__", "").replace("!", ""),".\\par"\
            "\\pard \\sa480 According to the GTDB R06-RS202, this order belongs to the class \i " + current_line[2].replace("c__", "").replace("!","") + \
            "\i0. \\par",\
            file=protologues)
  ...
  if "!" in current_line[2]: 
      if current_line[2].replace("c__", "").replace("ae!", "a") in current_line[5].replace("g__", ""): 
        if current_line[2] not in previous_line[2]:
          if "!" in current_line[1]: ## belongs to a newly named taxon, so old and new names of the higher taxon are included in protologue
            print \
            ...
            "\\pard \\sa120 \i Candidatus \i0", current_line[2].replace("!", "").replace("c__", ""), "(NL. fem. n. \i", current_line[2].replace("!", "").replace("o__", ""),\
            "\i0 a name created from the name of the type genus by addition of the appropriate suffix). \\par", \
            "\\pard \\sa120 A taxon identified and delineated according to the algorithms of the Genome Taxonomy Database (GTDB) release R06-RS202. The GTDB R06-RS202 placeholder designation for this taxon is \i{\doccomm oldname_2}", \
            current_line[10].replace("c__", "") + \
            "{\doccomm 2_oldname}. \i0 The type genus for the taxon is \i Candidatus \i0",\
            current_line[5].replace("g__", "").replace("!", ""),".\\par"\
            "\\pard \\sa480 According to the original version of GTDB R06-RS202, this family belongs to the phylum \i " + current_line[9].replace("p__", "").replace("!","") + \
            "\i0. According to the renamed version of GTDB R06-RS202, this taxon belongs to the phylum \i Candidatus \i0" + current_line[1].replace("p__", "").replace("!","") + \
            ". \\par",\
            file=protologues)
          else: ## belongs to an already named taxon, so no need to include names of the higher taxon from the original and renamed taxonomies in the protologue
            print \
            ...
            "{\doccomm 2_oldname}. \i0 The type genus for the taxon is \i Candidatus \i0",\
            current_line[5].replace("g__", "").replace("!", ""),".\\par"\
            "\\pard \\sa480 According to GTDB R06-RS202, this class belongs to the phylum \i " + current_line[1].replace("p__", "").replace("!","") + \
            "\i0. \\par",\
            file=protologues)
  ...
  if "!" in current_line[1]: 
      if current_line[1].replace("p__", "").replace("ota!", "a") in current_line[5].replace("g__", ""): 
       if current_line[1] not in previous_line[1]:
          ...
          "\\pard \\sa120 \i Candidatus \i0", current_line[1].replace("!", "").replace("p__", ""), "(NL. fem. n. \i", current_line[1].replace("!", "").replace("p__", ""),\
          "\i0 a name created from the name of the type genus by addition of the appropriate suffix). \\par", \
          "\\pard \\sa120 A taxon identified and delineated according to the algorithms of the Genome Taxonomy Database (GTDB) release R06-RS202. The GTDB R06-RS202 placeholder designation for this taxon is \i{\doccomm oldname_1}", \
          current_line[9].replace("p__", "") + \
          "{\doccomm 1_oldname}. \i0 The type genus for the taxon is \i Candidatus \i0",\
          current_line[5].replace("g__", "").replace("!", ""),".\\par"\
          "\\pard \\sa480 According to GTDB R06-RS202, this phylum belongs to the domain \i Bacteria \i0" \
          ". \\par",\
          file=protologues)
